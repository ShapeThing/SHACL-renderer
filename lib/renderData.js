...
import {
  ...
  SKIP_EFFECTS,
  VNODE
} from '../node_modules/preact-render-to-string/src/lib/constants.js'
import {
  createComponent,
  encodeEntities,
  HTML_ENUMERATED,
  ...
  SVG_CAMEL_CASE,
  UNSAFE_NAME
} from '../node_modules/preact-render-to-string/src/lib/util.js'
 ...
 */
export async function renderToDataAsync(vnode, context) {
  ...
  try {
    const rendered = await _renderToString(vnode, context || EMPTY_OBJ, false, undefined, parent, true, undefined)
    ...
    if (isArray(rendered)) {
      ...
      while (resolved.some(element => element && typeof element.then === 'function') && count++ < 25) {
...
function _renderToString(vnode, context, isSvgMode, selectValue, parent, asyncMode, renderer) {
  ...
  if (isArray(vnode)) {
    ...
    for (let i = 0; i < vnode.length; i++) {
      ...
      console.log(childRender, rendered)
  ...
  if (typeof type == 'function') {
    ...
    if (type === Fragment) {
      ...
      if (isClassComponent) {
        ...
        vnode[COMPONENT] = component = /**#__NOINLINE__**/ createComponent(vnode, cctx)
      ...
      if (
        ...
        let isTopLevelFragment =
          rendered != null && rendered.type === Fragment && rendered.key == null && rendered.props.tpl == null
    ...
    try {
      ...
      if (!asyncMode && renderer && renderer.onError) {
        let res = renderer.onError(error, vnode, child =>
          _renderToString(child, context, isSvgMode, selectValue, vnode, asyncMode, renderer)
        )
      ...
      const renderNestedChildren = () => {
        try {
          ...
          return e.then(
            () => _renderToString(rendered, context, isSvgMode, selectValue, vnode, asyncMode, renderer),
