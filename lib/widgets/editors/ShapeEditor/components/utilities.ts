...
export function getProjection(
  ...
  const parentIsValid =
    parentId === null || ['group', 'root'].includes(items.find(item => item.id === parentId)?.type ?? '')

  return { depth: parentIsValid ? depth : activeItem.depth, maxDepth, minDepth, parentId }
...
export function buildTree(flattenedItems: FlattenedItem[]): TreeItems {
  const root: TreeItem = { id: 'root', label: 'Root', type: 'root', children: [], term: dataFactory.namedNode('') }
  ...
  for (const item of items) {
    ...
    nodes[id] = { id, label, type, children, term: dataFactory.namedNode('') }
